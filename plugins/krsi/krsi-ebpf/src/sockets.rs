...
pub fn extract_sock_family(sk: *const vmlinux::sock) -> Result<u16, i64> {
    unsafe { bpf_probe_read_kernel(&(*sk).__sk_common.skc_family) }
}

/// Returns `sk->__sk_comm.skc_daddr`.
pub fn extract_sock_inet_daddr(sk: *const vmlinux::sock) -> Result<u32, i64> {
    unsafe {
...
pub fn extract_sock_inet_dport(sk: *const vmlinux::sock) -> Result<u16, i64> {
    unsafe {
        bpf_probe_read_kernel(
            &(*sk)
                .__sk_common
...
pub fn extract_sock_inet6_saddr(sk: *const vmlinux::inet_sock) -> Result<[u32; 4], i64> {
...
pub fn extract_sock_inet6_daddr(sk: *const vmlinux::sock) -> Result<[u32; 4], i64> {
    unsafe { bpf_probe_read_kernel(&(*sk).__sk_common.skc_v6_daddr.in6_u.u6_addr32) }
}

/// Returns `sk->inet_saddr`.
pub fn extract_inet_sock_saddr(sk: *const vmlinux::inet_sock) -> Result<u32, i64> {
    unsafe { bpf_probe_read_kernel(&(*sk).inet_saddr) }
}

/// Returns `sk->inet_sport`.
pub fn extract_inet_sock_sport(sk: *const vmlinux::inet_sock) -> Result<u16, i64> {
    unsafe { bpf_probe_read_kernel(&(*sk).inet_sport) }
}

/// Returns `sk->peer`.
pub fn extract_unix_sock_peer(
    sk: *const vmlinux::unix_sock,
) -> Result<*const vmlinux::unix_sock, i64> {
...
pub fn extract_unix_sock_addr_path_into(
    sk: *const vmlinux::unix_sock,
    is_kern_mem: bool,
    ...
    extract_sockaddr_un_path_into(name, is_kern_mem, path)
}

/// Returns `*sockaddr`.
fn extract_sockaddr_in(
...
pub fn extract_sockaddr_in_saddr_and_sport(
    sockaddr: *const vmlinux::sockaddr_in,
    is_kern_mem: bool,
) -> Result<(u32, u16), i64> {
    let sa = extract_sockaddr_in(sockaddr, is_kern_mem)?;
    Ok((sa.sin_addr.s_addr, sa.sin_port))
}

/// Returns `*sockaddr`.
fn extract_sockaddr_in6(
...
pub fn extract_sockaddr_in6_saddr_and_sport(
    sockaddr: *const vmlinux::sockaddr_in6,
    is_kern_mem: bool,
) -> Result<([u32; 4], u16), i64> {
    let sa = extract_sockaddr_in6(sockaddr, is_kern_mem)?;
    Ok((unsafe { sa.sin6_addr.in6_u.u6_addr32 }, sa.sin6_port))
}

/// Returns `*sockaddr`.
fn extract_sockaddr_un(
...
pub fn extract_sockaddr_un_path_into(
    sockaddr: *const vmlinux::sockaddr_un,
    is_kern_mem: bool,
    path: &mut [c_char; defs::UNIX_PATH_MAX],
) -> Result<(), i64> {
    let sa = extract_sockaddr_un(sockaddr, is_kern_mem)?;
    *path = sa.sun_path;
    Ok(())
}
