...
pub fn get_io_kiocb_cqe_res(
    ...
    let cqe_res = extract::io_kiocb_cqe_res(req)?;
    Ok(Some(cqe_res))
}

/// Returns the file descriptor leveraging the information extracted from `req->flags` and
/// `req->cqe.fd`.
pub fn get_io_kiocb_cqe_file_descriptor(
    req: *const vmlinux::io_kiocb,
) -> Result<FileDescriptor, i64> {
    let flags = extract::io_kiocb_flags(req)?;
    let fd = extract::io_kiocb_cqe_fd(req)?;
