...
bool my_plugin::extract(const falcosecurity::extract_fields_input &in)
{
    ...
    auto evt_type = evt_reader.get_type();

    bool is_container_async_event_create = false;
    bool is_container_async_event_remove = false;

    /*
     * NOTE: Extract might be called in two cases:
     * 1) event filtering (e.g. sinsp-example -f <filter>)
     * 2) event formatter (e.g. sinsp-example -o <fields>, falco rules)
     * For the events generated by us, in case 1 m_last_container will
     * be either null or wrong, because the present will be called before
     * parse_event(). Actually, the event might be filtered out, so
     * parse_event() won't be called at all. Case 2 is always fine.
     */
    ...
    if(is_container_event && m_last_container.first == evt_reader.get_ts())
    {
        cinfo = m_last_container.second;
    }
    else
    {
        try
        ...
        if(auto it = m_containers.find(container_id); it != m_containers.end())
        {
            cinfo = it->second;
        }
    }
    ...
    case TYPE_CONTAINER_ID:
        req.set_value((cinfo != nullptr) ? cinfo->m_id : container_id);
        break;
