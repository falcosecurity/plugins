...
bool my_plugin::parse_async_event(const falcosecurity::parse_event_input& in)
{
    ...
    if(added)
    {
        ...
        m_last_container = {evt.get_ts(), cinfo};
        ...
        const falcosecurity::parse_event_input& in)
{
    ...
    if(json_param.param_len == 0)
    {
        m_logger.log(fmt::format("Received container_json_2 event with empty "
                                 "`json` parameter. Cannot recover any "
                                 "container info. Skipping"),
                     falcosecurity::_internal::SS_PLUGIN_LOG_SEV_TRACE);
        return false;
    }
