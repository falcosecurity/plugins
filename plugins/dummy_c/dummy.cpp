...
public:
	...
	dummy_source(uint64_t max_evts, uint64_t start, uint64_t jitter) : m_event_count(0),
																	   m_max_evts(max_evts),
																	   m_sample_value(start),
																	   m_jitter(jitter),
																	   m_enc()
	{
	}
...
public:
	...
	{
		// We need to compile at least with c++11 to use an ordinary initializer list.
		...
		return {
			{ft::FTYPE_UINT64, "dummy.divisible", "Return 1 if the value is divisible by the provided divisor, 0 otherwise", "Return 1 if the value is divisible by the provided divisor, 0 otherwise", divisibile_arg},
			{ft::FTYPE_UINT64, "dummy.value", "The sample value in the event", "The sample value in the event"},
			{ft::FTYPE_STRING, "dummy.strvalue", "The sample value in the event, as a string", "The sample value in the event, as a string"},
		};
	}
	...
	bool extract(const falcosecurity::extract_fields_input &in)
	{
		...
		case 0: // dummy.divisible
		{
			...
			{
				log_error("'dummy.divisible' requires an argument but no argument is provided");
				return false;
			}
	...
	std::unique_ptr<dummy_source> open(const std::string &params)
	{
		...
		return std::unique_ptr<dummy_source>(new dummy_source(max_events, start, m_jitter));
	}
