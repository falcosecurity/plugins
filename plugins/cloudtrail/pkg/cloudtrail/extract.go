...
func getSessionNameFromAssumedRole(jdata *fastjson.Value) (bool, string, int, int) {
	...
	if principal != nil {
		...
		if len(parts) == 2 {
			return true, parts[1], principal.Offset(), principal.Len()
		}
	}
...
func getfieldStr(jdata *fastjson.Value, field string) (bool, string, int, int) {
	...
	case "ec2.imageid":
		zero := 0
		present, images, offset, length := getImageIds(jdata, &zero)
		if !present {
			return false, "", 0, 0
		}
...
func getfieldStrList(jdata *fastjson.Value, field string, argIndex *int) (bool, []string, int, int) {
	switch field {
	case "ec2.imageids":
		return getImageIds(jdata, argIndex)
	default:
		return false, nil, 0, 0
	}
}
