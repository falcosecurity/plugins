...
func (p *Plugin) extractOffsets(req sdk.ExtractRequest, evt sdk.EventReader) {
	// Read the event data
	data, err := io.ReadAll(evt.Reader())
	if err != nil {
		return
	...
	if ok {
		if path == "_GENERATED_" {
			req.SetValueOffsets(uint32(0), uint32(0))
		} else {
			...
			req.SetValueOffsets(uint32(sdk.PluginEventHeaderSize + start), uint32(len(result.Raw) + result.Index - start))
		}
	}
}
